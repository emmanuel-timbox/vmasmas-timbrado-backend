class MassiveCertificate < ApplicationRecord
  ...
  def self.get_certificate(slug_emitter)
    return MassiveCertificate.find_by(emitter_id: Emitter.find_by(slug: slug_emitter).id)
  end
  ...
  def self.insert_certificate(params)
    ...
    return { message: nil, data: MassiveCertificate.create(data) }
  end
  ...
  def self.update_certificate(params)
    ...
    destroy = MassiveCertificate.find_by(slug: params[:id]).destroy
